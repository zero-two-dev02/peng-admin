...
- 工作区：`D:\java-projects\yudao-cloud\yudao-admin`
...
2. 以当前交付任务为范围，连续完成相互依赖的页面、API、测试与构建，优先支持 Java 后端核心业务演示。
3. API 类型、请求地址和响应 JSON 属于前后端契约；不得猜测后端字段。
4. 默认由 Codex 完成页面结构、状态流转和核心交互，不保留教学空框架或知识检查；仅讨论或审查请求不自动授权实现。
...
- 当前按根项目交付计划执行；以上进度属于历史记录，现有页面及验收状态以当前源码与本次验证为准。
