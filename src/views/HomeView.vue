...
async function handleLoadSystemInfo() {
  ...
  try {
    ...
    systemMessage.value = 'System 服务不可达，请稍后重试。'
  } finally {
  ...
  <main class="dashboard-page">
    ...
    <section class="dashboard-stat-grid" aria-label="系统概览">
      <article class="dashboard-stat-card">
        ...
        <strong v-if="systemInfo" class="status-badge status-badge-success">运行正常</strong>
        <strong v-else-if="systemMessage" class="status-badge status-badge-warning">需要关注</strong>
        <strong v-else class="status-badge status-badge-neutral">正在检测</strong>
        <p v-if="systemInfo" class="dashboard-card-detail">{{ systemInfo }}</p>
        <p v-else-if="systemMessage" class="dashboard-card-detail">{{ systemMessage }}</p>
    ...
    <section class="dashboard-grid">
      <article class="dashboard-panel">
        <header class="dashboard-panel-header">
          ...
          <button class="text-button" type="button" @click="handleLoadCurrentUser">
            刷新账户信息
          </button>
        </header>
        <dl class="account-summary">
          ...
          <div>
            <dt>权限数量</dt>
            <dd>{{ currentUser?.permissions.length ?? authStore.permissions.length }}</dd>
          </div>
        </dl>
        <p v-if="message" class="form-message dashboard-message" role="status" aria-live="polite">
          {{ message }}
        </p>
        <button
      ...
      <article class="dashboard-panel">
        ...
        <nav class="dashboard-shortcuts" aria-label="系统管理快捷入口">
          ...
          <p v-if="!canReadUsers && !canReadRoles && !canReadPermissions" class="muted-text">
            当前账户暂无可访问的管理模块。
          </p>
        </nav>
      </article>
    ...
    <section v-if="authStore.hasSession" class="dashboard-panel dashboard-security-panel">
      <header class="dashboard-panel-header">
        <div>
      ...
      <form class="dashboard-password-form" @submit.prevent="handleUpdatePassword">
        <label class="form-field">
          <span>旧密码</span>
          <input v-model="oldPassword" type="password" autocomplete="current-password" minlength="8" maxlength="64" required />
        </label>
        <label class="form-field">
          <span>新密码</span>
          <input v-model="newPassword" type="password" autocomplete="new-password" minlength="8" maxlength="64" required />
        </label>
        <label class="form-field">
          <span>确认新密码</span>
          <input v-model="confirmNewPassword" type="password" autocomplete="new-password" minlength="8" maxlength="64" required />
        </label>
        <div class="dashboard-password-actions">
          ...
          <p v-if="passwordMessage" class="form-message" role="status" aria-live="polite">
            {{ passwordMessage }}
          </p>
        </div>
      </form>
