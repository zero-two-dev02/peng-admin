...
import { replacementSummary } from "../utils/safety";
...
const canWriteRoles = computed(() =>
  authStore.hasPermission("system:role:write"),
);
...
async function handleViewPermissions(role: RolePageItem) {
  if (
    ...
    isAssigningPermissions.value ||
    isDeletingRole.value
  )
    return;
  ...
  try {
    if (!canReadRolePermissions.value) {
      permissionMessage.value = "没有权限关系读取权限，不能安全替换。";
      return;
    }
    ...
    if (!authStore.hasPermission("system:permission:read")) {
      assignMessage.value = "没有权限目录读取权限，禁止分配。";
      return;
    }
    ...
    permissionMessage.value = errorMessage(failure);
  } finally {
...
async function loadRoleUsers() {
  ...
  try {
    const result = await getRoleUserPage({
      ...
      username: memberUsername.value || undefined,
      status:
        memberStatus.value === "" ? undefined : Number(memberStatus.value),
      pageNo: roleUserPageNo.value,
      pageSize,
    });

    if (result.code !== 0 || result.data === null) {
      roleUsersError.value = result.message;
      roleUsers.value = [];
      roleUserTotal.value = 0;
      return;
    }
    ...
    roleUsersError.value = errorMessage(failure);
    roleUsers.value = [];
    roleUserTotal.value = 0;
  } finally {
...
async function handleAssignPermissions() {
  if (
    ...
    isAssigningPermissions.value ||
    !relationsLoaded.value ||
    isLoadingPermissions.value ||
    selectedRole.value.status !== 0
  )
    return;
  isAssigningPermissions.value = true;
  if (
    !(await confirmAction(
      replacementSummary(permissionCodes.value, selectedPermissionCodes.value),
    ))
  ) {
  ...
  try {
    ...
    relationsLoaded.value = false;
    assignMessage.value = errorMessage(failure) + " 请重新加载关系。";
  } finally {
    isAssigningPermissions.value = false;
  }
}
...
async function handleDeleteRole() {
  ...
  if (
    !(await confirmAction(
      `确认永久删除角色 ${selectedRole.value.code} 吗？此操作不可恢复。`,
    ))
  ) {
...
<template>
  <main class="user-page management-page">
    <PageHeader
      title="角色管理"
      description="角色编码不可变；分配权限时确认完整集合的增删差异。"
      ><el-button v-if="canWriteRoles" type="primary" @click="createOpen = true"
        >创建角色</el-button
      ></PageHeader
    >
    <el-dialog
    ...
    >
      <section v-if="selectedRole" class="detail-panel management-detail-panel">
        ...
        >
          <h3>更新角色信息</h3>
          <label
            >角色名称<input v-model.trim="editName" type="text" maxlength="64"
          /></label>
          <label
            >状态<select v-model="editStatus">
              <option value="0">启用</option>
              <option value="1">停用</option>
            </select></label
          >
          <button class="login-button" type="submit" :disabled="isUpdatingRole">
            {{ isUpdatingRole ? "保存中..." : "保存角色信息" }}
          </button>
        ...
        <form
          ...
          class="inline-form"
          @submit.prevent="handleAssignPermissions"
        >
          <h3>分配权限</h3>
          ...
          <div class="checkbox-grid">
            ...
            >
              <input
                ...
                :disabled="isAssigningPermissions || !relationsLoaded"
              />
          ...
          <button
            ...
            "
          >
            {{ isAssigningPermissions ? "保存中..." : "保存权限分配" }}
          </button>
        ...
        <section class="inline-form">
          ...
          <form
            ...
            "
          >
            <label>成员用户名<input v-model.trim="memberUsername" /></label
            ><label
              >成员状态<select v-model="memberStatus">
                <option value="">全部</option>
                <option value="0">启用</option>
                <option value="1">停用</option>
              </select></label
            ><button type="submit" :disabled="isLoadingRoleUsers">
              查询成员
            </button>
          </form>
          <p v-if="roleUsersError" role="alert">{{ roleUsersError }}</p>
          ...
          <table v-else>
            ...
            <tbody>
              ...
              <tr v-if="roleUsers.length === 0">
                <td colspan="4">{{ roleUsersError || "暂无成员" }}</td>
              </tr>
            </tbody>
          </table>
          <footer class="pagination">
            ...
            <button
              type="button"
              :disabled="
                roleUserPageNo >= roleUserTotalPages || isLoadingRoleUsers
              "
        ...
        <section v-if="canDeleteRoles" class="inline-form danger-zone">
          <h3>删除角色</h3>
          <p class="warning-text">
            仅能删除未关联用户的自定义角色；内置角色会被后端拒绝。
          </p>
          <button
            class="danger-button"
            type="button"
